...
\begin{center} false, because although the values are the same, the LHS returns a matrix while the RHS returns a value \end{center}
\item $x^Tx = \norm{x}^2$.
\begin{center} false, because although the values are the same, the LHS returns a matrix while the RHS returns a value \end{center}
\item $x^Tx = xx^T$.
\begin{center} false \end{center}
\item $(x-y)^T(x-y) = \norm{x}^2 - 2x^Ty + \norm{y}^2$.
\begin{center} false \end{center}
\item $AB=BA$.
\begin{center} false \end{center}
\item $A^T(B + IC) = A^TB + A^TC$.
\begin{center} true \end{center}
...
\begin{center} $f'(x) = 6x - 2.$ \end{center}
\item Find the derivative of the function $f(x) = x(1-x)$.
\begin{center} $f'(x) = 1-2x$ \end{center}
...
\begin{center} $f'(x) = 1-\frac{\exp(-x)}{p(x)}.$ \end{center}
\end{enumerate}
...
\begin{center} $O(n)$ \end{center}
\item What is the cost of finding the smallest element greater than 0 in a \emph{sorted} list with $n$ numbers.
\begin{center} $O(\log n)$ \end{center}
...
\begin{center} maximum = 4.862, minimum = 0.352, median = 1.177, mean = 1.324625, mode = 0.77 \end{center} 
\item The $5\%$, $25\%$, $50\%$, $75\%$, and $95\%$ quantiles of all values across the dataset.
\begin{center} $5\% = 0.466, 25\% = 0.72, 50\% = 1.177, 75\% = 1.8145, 95\% = 2.646$ \end{center}
\item The names of the regions with the highest and lowest means, and the highest and lowest variances.
\begin{center} highest mean and variance = WtdILI, lowest mean and variance = Pac \end{center}
}
...
\begin{center}
    Plot C: histogram showing only the columns (regions) of the dataset
\end{center}
\item A histogram showing the distribution of each the values in the matrix $X$.
\begin{center}
    Plot D: histogram that displays ALL the values
\end{center}
\item A boxplot grouping data by weeks, showing the distribution across regions for each week.
\begin{center}
    Plot B: Boxplot that has an x-axis in weeks
\end{center}
\item A plot showing the illness percentages over time.
\begin{center}
    Plot A: y-axis shows the illness percentage
\end{center}
\item A scatterplot between the two regions with highest correlation.
\begin{center}
    Plot F: scatterplot that has a better linear distribution which means better correlation
\end{center}
\item A scatterplot between the two regions with lowest correlation.
\begin{center}
    Plot E: scatterplot where points show a slight linear distribution which means its less correlated
\end{center}
}
